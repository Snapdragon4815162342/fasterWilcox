...
DataFrame sparse_fasterWilcox_old(Rcpp::S4 A,
  ...
  std::cout<<"\ntmp1s: ";
  for(size_t pp=0; pp<tmp1s.size(); pp++)
    for(size_t qq=0;qq<tmp1s.size(); qq++){
    std::cout<<tmp1s[pp][qq]<<" ";
      cout<<endl;
    }
  std::cout<<"\ntmp1s: ";
  for(size_t pp=0; pp<tmp2s.size(); pp++)
    for(size_t qq=0;qq<tmp2s.size(); qq++){
      std::cout<<tmp2s[pp][qq]<<" ";
      cout<<endl;
    }
